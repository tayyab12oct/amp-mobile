...
const ListicleDetails = ({ staticListiclesProps }) => {
          ...
          firebaseAnalytics.logEvent('screen_view', {
            page_title:
              ...
              (response.data.result[0] && response.data.result[0].content_type
                ...
                : ''),
            page_location:
              '/listiclesDetail' + response.data.result[0] &&
          ...
          __html: JSON.stringify({
            ...
            articleBody:
              staticListiclesProps && staticListiclesProps[0]?.description
                ? `${removeAllHTMLTags(staticListiclesProps[0].description)} ${
                    getMovieList().length > 0 ? getMovieList().join(', ') : ''
                  }`
                : 'Listicles',
            ...
            description: staticListiclesProps[0]?.meta_description
              ? removeAllHTMLTags(staticListiclesProps[0].meta_description)
            ...
            author: {
              '@type': 'Person',
              name:
                staticListiclesProps && staticListiclesProps[0]?.byline
                  ? removeAllHTMLTags(staticListiclesProps[0].byline)
                  : 'Not available',
            },
            image: {
              '@type': 'ImageObject',
    ...
    return (
      <React.Fragment>
        <SEO
          ...
          ogData={{
            ogTitle: staticListiclesProps[0]?.name
              ? staticListiclesProps[0].name
              : staticListiclesProps[0]?.meta_title
              ? staticListiclesProps[0].meta_title
              : 'Top OTT Article',
            ogDescription: staticListiclesProps[0]?.description
              ? staticListiclesProps[0].description
...
export async function getStaticProps(context) {
  ...
  if (
    context &&
    context.params &&
    context.params.listicleslug[2] === 'preview'
  ) {
    typeData = 'preview';
  }
  const pagePath = `${context.params.listicleslug[0]}/${context.params.listicleslug[1]}`;
  const params = {
    seoUrl: pagePath,
...
export default ListicleDetails;
