...
export default function Cast() {
  ...
  React.useEffect(() => {
    ...
    state.loadingData && state.page > 1 && getCastList();
  }, [state.loadingData]);
  ...
  const getCastList = () => {
    ...
    webfox.getCastOnboardingList(params).then(({ data, error }) => {
      ...
      if (data && data.rank && data.rank.length > 0) {
        ...
        if (webstore.cast.toggle) {
          data.rank
            .filter((e) => e.cast['status'] === 'published' && e.cast.headshot)
            .map((item, index) => {
              ...
              <label className={classes.selectAll} htmlFor="refine-switch">
                ...
                <Switch
                  checked={
                    ...
                    webstore.cast.name.length === state.result.length
                      ? true
