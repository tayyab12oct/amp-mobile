...
export default function Shows({ movie, ...props }) {
  ...
  useEffect(() => {
    firebaseAnalytics.logEvent('shows', {
      screen_view:
        '/shows' +
        '/' +
    ...
    windowAny.Moengage &&
      windowAny.Moengage.track_event('shows', {
        screen_view: '/shows',
  ...
  const apiForShows = (params) => {
    setState({
      loadingData: true,
    });
    webfox.getAllShows(params).then(({ data, error }) => {
      // console.log('Movies result. ', JSON.stringify(data));
      if (error) {
        actionDispatch(actions.FETCH_SHOWS_FAILURE, []);
      }
      if (data && data.shows) {
        setState({
          result:
            state.result.length > 0
              ? showList.data.concat(data.shows)
              : data.shows,
          ...
          unSortedResults:
            state.unSortedResults.length > 0
              ? state.unSortedResults.concat(data.shows)
              : [...data.shows],
          page: state.page + 1,
          loadingData: false,
        });
  ...
  const handleFilters = (button) => {
    ...
    webfox.getAllShows(params).then(({ data, error }) => {
      ...
      if (data && data.shows) {
        ...
        actionDispatch(actions.FETCH_SHOWS_SUCCESS, data.shows);
      } else {
        setState({
          loadingData: false,
        });
  ...
  const { showList } = webstore;

  useEffect(() => {
    setState({
      result: showList.data,
    });
  }, [showList.data]);
  ...
  //console.log('resss', webstore.showList.data.shows);
          ...
          <div className={classes.headerText}>
            {state.totalData > 0 && (
              <React.Fragment>
                ...
                <div style={{ paddingLeft: '5px' }}>show titles</div>
              </React.Fragment>
            )}
          </div>
          <ListingHeader
            ...
            >
              {!state.loadingData && state.result.length === 0 ? (
                ...
                <div>
                  ...
                  {state.loadingData && (
                    <Spinner
                      styles={{ height: state.page === 1 ? '60vh' : '5vh' }}
                    />
                  )}
                </div>
              )}
