...
export default function MovieShowDetails({
  ...
  staticTvShowProps &&
    staticTvShowProps[0] &&
    staticTvShowProps[0]?.casts?.map((item) => {
      casts.push(item);
    });
  staticTvShowProps &&
    staticTvShowProps[0] &&
    staticTvShowProps[0]?.crews?.map((item) => {
      casts.push(item);
    });
  ...
  const data =
    staticTvShowProps && staticTvShowProps[0] && staticTvShowProps[0]?.name;

  const cast: any = [];
  const crew: any = [];
  staticTvShowProps &&
    staticTvShowProps[0] &&
    staticTvShowProps[0]?.casts?.map((c) => {
  ...
  staticTvShowProps &&
    staticTvShowProps[0] &&
    staticTvShowProps[0]?.crews?.map((c) => {
  ...
  const handleReviewClick = (contentType, url, seoUrl) => {
    if (contentType === 'critics') {
      ...
      router.push({
        ...
        // pathname: `/${staticTvShowProps[0]['name']
        //   .toLowerCase()
  ...
  const type =
    staticTvShowProps &&
    staticTvShowProps[0] &&
    staticTvShowProps[0]?.content_type &&
    staticTvShowProps[0]?.content_type;
  ...
  const handleWhereToWatchClick = () => {
    const params = {
      ...
      details: {
        ...
        // name: staticTvShowProps[0] && staticTvShowProps[0].name,
        // formatted_id:
        //   staticTvShowProps[0].primary_language &&
        //   staticTvShowProps[0].primary_language.name + '_' + staticTvShowProps[0]._id,
        content_type: props && props.type ? props.type : '',
        name:
          staticTvShowProps &&
          staticTvShowProps[0] &&
          staticTvShowProps[0]?.name
            ? staticTvShowProps[0]?.name
            : '',
        formatted_id:
          staticTvShowProps &&
          staticTvShowProps[0] &&
          staticTvShowProps[0]?.primary_language &&
          staticTvShowProps[0]?.primary_language !== undefined &&
          staticTvShowProps[0]?.primary_language?.name &&
          staticTvShowProps[0]?._id
            ? staticTvShowProps[0].primary_language.name +
              '_' +
              staticTvShowProps[0]._id
            : '',
    ...
    firebaseAnalytics.logEvent(
      props.type === 'movie' ? 'moviewatchnowcta' : 'showwatchnowcta',
      {
        ...
        eventLabel:
          staticTvShowProps[0]?.name +
          (staticTvShowProps[0]?.primary_language &&
          staticTvShowProps[0]?.primary_language.name != undefined
            ? `/${staticTvShowProps[0]?.primary_language?.name}`
            : '') +
          (staticTvShowProps[0]?._id
            ? '/' + staticTvShowProps[0]?._id
            : '/' + staticTvShowProps[0]?.id),
        eventValue: staticTvShowProps[0]?.ottplay_rating,
      }
    );
  ...
  const getCastArr = () => {
    ...
    staticTvShowProps &&
      staticTvShowProps[0] &&
      staticTvShowProps[0]?.casts?.length &&
      staticTvShowProps[0].casts.map((item) => {
        castsArr.push({
          '@type': 'Person',
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 3,
                name:
                  staticTvShowProps[0] && staticTvShowProps[0]?.name
                    ? staticTvShowProps[0]?.name
                    : 'Not available',
                url:
                  'http://www.ottplay.com/' +
                  (staticTvShowProps[0] && staticTvShowProps[0]?.permalink
                    ? staticTvShowProps[0].permalink
                    : ''),
              },
            ],
          }),
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            '@type': 'Movie',
            url:
              staticTvShowProps[0] && staticTvShowProps[0]?.seo_url
                ? staticTvShowProps[0].seo_url
                : 'https://www.ottplay.com',
            name:
              staticTvShowProps[0] && staticTvShowProps[0]?.name
                ? staticTvShowProps[0].name
                : 'Not available',
            image: staticTvShowProps[0]?.tmdb_posters
              ? staticTvShowProps[0].tmdb_posters[0]
              : staticTvShowProps[0]?.posters
              ? staticTvShowProps[0]?.posters[0]
              : 'www.ottplay.com',
            genre:
              staticTvShowProps[0] && staticTvShowProps[0]?.genres?.length > 0
                ? staticTvShowProps[0].genres.map((i) => i.name)
                : null,
            contentRating:
              staticTvShowProps[0] &&
              staticTvShowProps[0]?.certifications &&
              staticTvShowProps[0]?.certifications?.certification
                ? staticTvShowProps[0].certifications?.certification
                : 'Not available',
            actor: getCastArr().length > 0 ? [...getCastArr()] : [],
            director: { ...dirObj },
            creator: getCastArr().length > 0 ? [...getCastArr()] : [],
            description: staticTvShowProps[0]?.seo?.meta_description
              ? removeAllHTMLTags(staticTvShowProps[0].seo.meta_description)
              : staticTvShowProps[0]?.full_synopsis
              ? removeAllHTMLTags(staticTvShowProps[0].full_synopsis)
              : 'Not available',
            datePublished:
              staticTvShowProps[0] &&
              staticTvShowProps[0]?.onboarding_updated_on
                ? new Date(staticTvShowProps[0].onboarding_updated_on)
                    .toString()
                    .slice(4)
                : new Date().toString().slice(4),
            keywords:
              staticTvShowProps[0]?.seo &&
              staticTvShowProps[0]?.seo?.meta_keywords &&
              staticTvShowProps[0]?.seo?.meta_keywords?.length > 0
                ? staticTvShowProps[0].seo.meta_keywords.join(',')
                : null,
            aggregateRating: {
              ...
              worstRating: 1.0,
              ratingValue:
                staticTvShowProps[0] && staticTvShowProps[0]?.ottplay_rating
                  ? staticTvShowProps[0].ottplay_rating
                  : 0.0,
            },
            duration:
              'PT' +
              Math.floor(staticTvShowProps[0]?.run_time / 60) +
              'H' +
              (staticTvShowProps[0]?.run_time % 60) +
              'M',
            trailer: {
              '@type': 'VideoObject',
              ...
              embedUrl:
                staticTvShowProps[0] &&
                staticTvShowProps[0]?.videos_trailers[0] &&
                staticTvShowProps[0]?.videos_trailers[0]?.video_url
                  ? staticTvShowProps[0].videos_trailers[0].video_url
                  : 'www.ottplay.com',
              thumbnail: {
                '@type': 'ImageObject',
              ...
              thumbnailUrl:
                staticTvShowProps[0] &&
                staticTvShowProps[0]?.videos_trailers[0] &&
                staticTvShowProps[0]?.videos_trailers[0]?.thumbnail_url
                  ? staticTvShowProps[0].videos_trailers[0].thumbnail_url
                  : 'www.ottplay.com',
              description:
                staticTvShowProps[0] &&
                staticTvShowProps[0]?.videos_trailers[0] &&
                staticTvShowProps[0]?.videos_trailers[0]?.title
                  ? staticTvShowProps[0].videos_trailers[0].title
                  : staticTvShowProps[0]?.seo &&
                    staticTvShowProps[0]?.seo?.meta_description
                  ? staticTvShowProps[0]?.seo?.meta_description
                  : staticTvShowProps[0] && staticTvShowProps[0]?.name
                  ? staticTvShowProps[0]?.name + ' trailer'
                  : 'OTTPlay',
              uploadDate:
                staticTvShowProps[0] &&
                staticTvShowProps[0]?.onboarding_updated_on
  ...
  if (staticTvShowProps)
    return (
      <React.Fragment>
        {staticTvShowProps && staticTvShowProps.length > 0 ? (
          <SEO
            exposeToGoogle={
              false
              // staticTvShowProps &&
              // staticTvShowProps[0] &&
              // staticTvShowProps[0]?.is_crawlable
            }
            title={`Show - ${router.asPath.split('/')[2]}`}
            ogData={{
              ogTitle: staticTvShowProps[0]?.name
                ? staticTvShowProps[0].name
                : '',
              ogDescription: staticTvShowProps[0]?.full_synopsis
                ? staticTvShowProps[0].full_synopsis
                : staticTvShowProps[0]?.seo?.meta_description
                ? staticTvShowProps[0].seo.meta_description
                : null,

              ogImage: getImageUrl(staticTvShowProps[0]),
            }}
            metaData={{
              metaTitle: staticTvShowProps[0]?.seo?.meta_title
                ? staticTvShowProps[0].seo.meta_title
                : `${
                    staticTvShowProps[0]?.name ? staticTvShowProps[0].name : ''
                  } ${
                    staticTvShowProps[0]?.release_year
                      ? staticTvShowProps[0].release_year
                      : ''
                  } watch movie streaming online on ${getWTWlist(
                    staticTvShowProps[0]?.where_to_watch
                      ? staticTvShowProps[0].where_to_watch
                      : []
                  )}`,
              metaDescription: staticTvShowProps[0]?.seo?.meta_description
                ? staticTvShowProps[0].seo.meta_description
                : staticTvShowProps[0]?.full_synopsis
                ? staticTvShowProps[0].full_synopsis
                : null,
            ...
            microData={{
              name: staticTvShowProps[0]?.name ? staticTvShowProps[0].name : '',
              description: staticTvShowProps[0]?.seo?.meta_description
                ? removeAllHTMLTags(staticTvShowProps[0].seo.meta_description)
                : staticTvShowProps[0]?.full_synopsis
                ? removeAllHTMLTags(staticTvShowProps[0].full_synopsis)
              ...
              editor: 'OTTPlay',
              headline: staticTvShowProps[0]?.name
                ? staticTvShowProps[0].name
                : '',
              inLanguage: 'English',
            ...
            schema={[
              ...
              staticTvShowProps[0] &&
              staticTvShowProps[0].content_type &&
              staticTvShowProps[0].content_type === 'movie'
                ? movieSchema()
                : showSchema(),
              WebPageSchema(
                staticTvShowProps[0] && staticTvShowProps[0]?.name
                      ...
                      <>
                        ...
                        <Hidden only={['xs']}>
                          ...
                          />
                            ...
                            {/* Hidden */}
                            {/* {staticTvShowProps[0]['reviews'] &&
                        staticTvShowProps[0]['reviews'].length > 0 ? (
                          <Grid item xs={12} className={classes.reviewBox}>
                            ...
                            {/* <Hidden only={['xs']}> */}
                            {staticTvShowProps[0]['reviews'] &&
                            staticTvShowProps[0]['reviews'].filter(
                              (review) =>
                              ...
                              <>
                                ...
                                <CriticsReview
                                  criticsReviews={staticTvShowProps[0][
                                    'reviews'
                                  ].filter(
                                    (review) =>
                                      review.headline !== '' &&
...
export async function getStaticProps(context) {
  let typeData;
  if (context && context.params && context.params.tsslug[2] === 'preview') {
    typeData = 'preview';
  }
  const pagePath = `${context.params.tsslug[0]}/${context.params.tsslug[1]}`;
  const params = {
    seoUrl: pagePath,
  ...
  return {
    props: {
      staticTvShowProps: (await res.data.shows) || [],
    },
  };
}
export async function getStaticPaths(props) {
  ...
  await getMovies.response?.data.shows.map((item) => {
    data.push([{ params: { tsslug: item.seo_url } }]);
  });
  return {
    paths: data,
    fallback: 'blocking',
