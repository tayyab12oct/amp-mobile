...
export default function MovieShowDetails({
  ...
  React.useEffect(() => {
    ...
    //console.log("staticWebSeriesProps query", query);
    {
  ...
  const casts: any = [];
  staticWebSeriesProps &&
    staticWebSeriesProps[0] &&
    staticWebSeriesProps[0]?.casts?.map((item) => {
      casts.push(item);
    });
  staticWebSeriesProps &&
    staticWebSeriesProps[0] &&
    staticWebSeriesProps[0]?.crews?.map((item) => {
      casts.push(item);
    });
  ...
  const data =
    staticWebSeriesProps &&
    staticWebSeriesProps[0] &&
    staticWebSeriesProps[0]?.name;

  const cast: any = [];
  const crew: any = [];
  staticWebSeriesProps &&
    staticWebSeriesProps[0] &&
    staticWebSeriesProps[0]?.casts?.map((c) => {
  ...
  staticWebSeriesProps &&
    staticWebSeriesProps[0] &&
    staticWebSeriesProps[0]?.crews?.map((c) => {
  ...
  const getCreator = () => {
    const crews: any = [];
    if (staticWebSeriesProps[0] && staticWebSeriesProps[0]?.crews) {
      staticWebSeriesProps[0].crews.map((c) => {
        if (
          c.role_name &&
          (c.role_name === 'Creator' ||
  ...
  const handleEpisodesAll = (
    ...
    actionDispatch(actions.SET_SEEALL_EPISODES, {
      key,
      view,
      name,
      genres,
  ...
  const handleReviewClick = (contentType, url, seoUrl) => {
    if (contentType === 'critics') {
      ...
      router.push({
        ...
        // pathname: `/${staticWebSeriesProps[0]['name']
        //   .toLowerCase()
  ...
  const type =
    staticWebSeriesProps &&
    staticWebSeriesProps[0] &&
    staticWebSeriesProps[0]?.content_type;
  ...
  const handleWhereToWatchClick = () => {
    const params = {
      ...
      details: {
        ...
        // name: staticWebSeriesProps[0] && staticWebSeriesProps[0].name,
        // formatted_id:
        //   staticWebSeriesProps[0].primary_language &&
        //   staticWebSeriesProps[0].primary_language.name + '_' + staticWebSeriesProps[0]._id,
        ...
        formatted_id:
          ...
          staticWebSeriesProps[0] &&
          staticWebSeriesProps[0]?.primary_language &&
          staticWebSeriesProps[0]?.primary_language !== undefined &&
          staticWebSeriesProps[0]?.primary_language.name &&
          staticWebSeriesProps[0]?._id
            ? staticWebSeriesProps[0].primary_language.name +
              '_' +
              staticWebSeriesProps[0]._id
            : '',
    ...
    firebaseAnalytics.logEvent(
      props.type === 'movie' ? 'moviewatchnowcta' : 'showwatchnowcta',
      {
        ...
        eventLabel:
          staticWebSeriesProps[0]?.name +
          (staticWebSeriesProps[0]?.primary_language &&
          staticWebSeriesProps[0]?.primary_language.name != undefined
            ? `/${staticWebSeriesProps[0]?.primary_language?.name}`
            : '') +
          (staticWebSeriesProps[0]?._id
            ? '/' + staticWebSeriesProps[0]?._id
            : '/' + staticWebSeriesProps[0]?.id),
        eventValue: staticWebSeriesProps[0]?.ottplay_rating,
      }
    );
  ...
  const getCastArr = () => {
    const castsArr: any = [];
    staticWebSeriesProps[0] &&
      staticWebSeriesProps[0]?.casts?.length &&
      staticWebSeriesProps[0]?.casts?.map((item) => {
        castsArr.push({
          '@type': 'Person',
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              ...
              },
              {
                ...
                position: 3,
                name:
                  staticWebSeriesProps[0] && staticWebSeriesProps[0]?.name
                    ? staticWebSeriesProps[0].name
                    : 'show',
                url:
                  'http://www.ottplay.com/' +
                  (staticWebSeriesProps[0] && staticWebSeriesProps[0]?.permalink
                    ? staticWebSeriesProps[0].permalink
                    : ''),
              },
            ],
          }),
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            '@type': 'Movie',
            url:
              staticWebSeriesProps[0] && staticWebSeriesProps[0]?.seo_url
                ? staticWebSeriesProps[0].seo_url
                : 'https://www.ottplay.com',
            name:
              staticWebSeriesProps[0] && staticWebSeriesProps[0]?.name
                ? staticWebSeriesProps[0].name
                : 'Not available',
            image: staticWebSeriesProps[0]?.tmdb_posters
              ? staticWebSeriesProps[0].tmdb_posters[0]
              : staticWebSeriesProps[0]?.posters
              ? staticWebSeriesProps[0]?.posters[0]
              : 'www.ottplay.com',
            genre:
              staticWebSeriesProps[0] &&
              staticWebSeriesProps[0]?.genres?.length > 0
                ? staticWebSeriesProps[0].genres.map((i) => i.name)
                : null,
            contentRating:
              staticWebSeriesProps[0] &&
              staticWebSeriesProps[0]?.certifications &&
              staticWebSeriesProps[0]?.certifications?.certification
                ? staticWebSeriesProps[0].certifications?.certification
                : 'Not available',
            actor: getCastArr().length > 0 ? [...getCastArr()] : [],
            director: { ...dirObj },
            creator: getCastArr().length > 0 ? [...getCastArr()] : [],
            description: staticWebSeriesProps[0]?.seo?.meta_description
              ? removeAllHTMLTags(staticWebSeriesProps[0].seo.meta_description)
              : staticWebSeriesProps[0]?.full_synopsis
              ? removeAllHTMLTags(staticWebSeriesProps[0].full_synopsis)
              : 'Not available',
            datePublished:
              staticWebSeriesProps[0] &&
              staticWebSeriesProps[0]?.onboarding_updated_on
                ? new Date(staticWebSeriesProps[0].onboarding_updated_on)
                    .toString()
                    .slice(4)
                : new Date().toString().slice(4),
            keywords:
              staticWebSeriesProps[0]?.seo &&
              staticWebSeriesProps[0]?.seo?.meta_keywords &&
              staticWebSeriesProps[0]?.seo?.meta_keywords?.length > 0
                ? staticWebSeriesProps[0].seo.meta_keywords.join(',')
                : null,
            aggregateRating: {
              ...
              ratingValue:
                staticWebSeriesProps[0] &&
                staticWebSeriesProps[0]?.ottplay_rating
                  ? staticWebSeriesProps[0].ottplay_rating
                  : 0.0,
            },
            duration:
              'PT' +
              Math.floor(staticWebSeriesProps[0]?.run_time / 60) +
              'H' +
              (staticWebSeriesProps[0]?.run_time % 60) +
              'M',
            trailer: {
              '@type': 'VideoObject',
              ...
              embedUrl:
                staticWebSeriesProps[0] &&
                staticWebSeriesProps[0]?.videos_trailers[0] &&
                staticWebSeriesProps[0]?.videos_trailers[0]?.video_url
                  ? staticWebSeriesProps[0].videos_trailers[0].video_url
                  : 'www.ottplay.com',
              thumbnail: {
                '@type': 'ImageObject',
              ...
              thumbnailUrl:
                staticWebSeriesProps[0] &&
                staticWebSeriesProps[0]?.videos_trailers[0] &&
                staticWebSeriesProps[0]?.videos_trailers[0]?.thumbnail_url
                  ? staticWebSeriesProps[0].videos_trailers[0].thumbnail_url
                  : 'www.ottplay.com',
              description:
                staticWebSeriesProps[0] &&
                staticWebSeriesProps[0]?.videos_trailers[0] &&
                staticWebSeriesProps[0]?.videos_trailers[0]?.title
                  ? staticWebSeriesProps[0].videos_trailers[0].title
                  : staticWebSeriesProps[0]?.seo &&
                    staticWebSeriesProps[0]?.seo?.meta_description
                  ? staticWebSeriesProps[0]?.seo?.meta_description
                  : staticWebSeriesProps[0] && staticWebSeriesProps[0]?.name
                  ? staticWebSeriesProps[0]?.name + ' trailer'
                  : 'OTTPlay',
              uploadDate:
                staticWebSeriesProps[0] &&
                staticWebSeriesProps[0]?.onboarding_updated_on
  ...
  if (staticWebSeriesProps)
    return (
      <React.Fragment>
        {staticWebSeriesProps && staticWebSeriesProps.length > 0 ? (
          <SEO
            exposeToGoogle={
              false
              // staticWebSeriesProps &&
              // staticWebSeriesProps[0] &&
              // staticWebSeriesProps[0]?.is_crawlable
            }
            title={`Web Series - ${router.asPath.split('/')[2]}`}
            ogData={{
              ogTitle: staticWebSeriesProps[0]?.name
                ? staticWebSeriesProps[0].name
                : '',
              ogDescription: staticWebSeriesProps[0]?.full_synopsis
                ? staticWebSeriesProps[0].full_synopsis
                : staticWebSeriesProps[0]?.seo?.meta_description
                ? staticWebSeriesProps[0].seo.meta_description
                : null,

              ogImage: getImageUrl(staticWebSeriesProps[0]),
            }}
            metaData={{
              metaTitle: staticWebSeriesProps[0]?.seo?.meta_title
                ? staticWebSeriesProps[0].seo.meta_title
                : `${
                    staticWebSeriesProps[0]?.name
                      ? staticWebSeriesProps[0].name
                      : ''
                  } ${
                    staticWebSeriesProps[0]?.release_year
                      ? staticWebSeriesProps[0].release_year
                      : ''
                  } watch movie streaming online on ${getWTWlist(
                    staticWebSeriesProps[0]?.where_to_watch
                      ? staticWebSeriesProps[0].where_to_watch
                      : []
                  )}`,
              metaDescription: staticWebSeriesProps[0]?.seo?.meta_description
                ? staticWebSeriesProps[0].seo.meta_description
                : staticWebSeriesProps[0]?.full_synopsis
                ? staticWebSeriesProps[0].full_synopsis
                : null,
            ...
            microData={{
              name: staticWebSeriesProps[0]?.name
                ? staticWebSeriesProps[0].name
                : '',
              description: staticWebSeriesProps[0]?.seo?.meta_description
                ? removeAllHTMLTags(
              ...
              editor: 'OTTPlay',
              headline: staticWebSeriesProps[0]?.name
                ? staticWebSeriesProps[0].name
                : '',
              inLanguage: 'English',
            ...
            schema={[
              ...
              staticWebSeriesProps[0] &&
              staticWebSeriesProps[0].content_type &&
              staticWebSeriesProps[0].content_type === 'movie'
                ? movieSchema()
                : showSchema(),
              WebPageSchema(
                staticWebSeriesProps[0] && staticWebSeriesProps[0]?.name
        ...
        >
          <Grid xs={12}>
            ...
            {loadingData ? (
              ...
              <>
                {staticWebSeriesProps &&
                  staticWebSeriesProps.map((details) => {
                    const array =
                      JSON.parse(
                        typeof window !== 'undefined' &&
                      ...
                      <>
                        ...
                        <Hidden only={['xs']}>
                          ...
                          />
                            ...
                            {/* Hidden */}
                            {/* {staticWebSeriesProps[0]['reviews'] &&
                        staticWebSeriesProps[0]['reviews'].length > 0 ? (
                          <Grid item xs={12} className={classes.reviewBox}>
                            ...
                            {/* <Hidden only={['xs']}> */}
                            {staticWebSeriesProps[0]['reviews'] &&
                            staticWebSeriesProps[0]['reviews'].filter(
                              (review) =>
                              ...
                              <>
                                ...
                                <CriticsReview
                                  criticsReviews={staticWebSeriesProps[0][
                                    'reviews'
                                  ].filter(
                                    (review) =>
                                      review.headline !== '' &&
...
export async function getStaticProps(context) {
  let typeData;
  if (context && context.params && context.params.wsslug[2] === 'preview') {
    typeData = 'preview';
  }
  const pagePath = `${context.params.wsslug[0]}/${context.params.wsslug[1]}`;
  const params = {
    seoUrl: pagePath,
  ...
  return {
    props: {
      staticWebSeriesProps: await res.data.shows,
    },
  };
}
export async function getStaticPaths(props) {
  ...
  await getMovies.response?.data.shows.map((item) => {
    data.push([{ params: { wsslug: item.seo_url } }]);
  });
  return {
    paths: data,
    fallback: 'blocking',
