...
import { CastCrewDescription } from '../../components/CastCrewDescription';
...
export default function CastCrewDetails({ location }) {
  ...
  useEffect(() => {
    const seoId = router.asPath;
    const seoKey =
      seoId.split('/')[2] + '/' + seoId.split('/')[3].split('?')[0];
    typeof window !== 'undefined' && window.scrollTo(0, 0);
    //console.log('seoKey', seoId);
    const query = {
      seoUrl: seoKey,
    };
                    ...
                    {detail.name ? (
                      <>
                        ...
                        <CastCrewDescription
                          ...
                          description={
                            detail.is_original_bio &&
                            detail.full_bio &&
                            encodeURIComponent(detail.full_bio)
                            // .replace(/(<([^>]+)>)/gi, '')
                            // .replace(/&nbsp;/gi, '')
                            // .replace(/[!&#$%*@;+-=_][0-9][;&#]/gi, '')
                            // .replace(/&#/g, '')
...
export async function getStaticPaths(props) {
  ...
  await getMovies.response?.data.movies.map((item) => {
    ...
    const url = item.seo_url;
    const getPeoples = webfox.getMoviesDetailsList(url);
    const castList = getPeoples.response.data.result.casts.cast;
    const crewList = getPeoples.response.data.result.crews.cast;
    const people = castList.concate(crewList);

    people.map((people) => {
      data.push([{ params: { peopleslug: people.seo_url } }]);
    });
  });
