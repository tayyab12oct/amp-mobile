...
import { NotFound } from './ErrorScreen';
...
export default function GlobalSearch() {
  ...
  const observer: { current: any } = React.useRef();
  ...
  const params = {
    ...
    language: localStorage.getItem('languages'),
    limit: 36,
    page: state.page,
  };
  ...
  useEffect(() => {
    firebaseAnalytics.logEvent('search', {
      screen_view:
        '/search/' +
        (webstore.recents.history ? '/' + 'recent-yes' : '/' + 'recent-no') +
        '/' +
        state.searchItem +
        '/' +
    ...
    windowAny.Moengage.track_event('search', {
      screen_view: '/search/',
  ...
  const Recents = () => {
    const handleCard = (key, type, detail, name, s_url, p_url) => {
      ...
      Router.push({
        pathname: `/${url}`,
        query: { key, type, name, source: 'Search' },
      });
    };
      ...
      >
        ...
        <Grid xs={12} container>
          ...
          <Grid xs={12} sm={10} lg={8}>
            ...
            >
              {state.searchItem.length > 2 && state.search.length > 0 ? (
                <GenreButtons
                  button={state.button}
                  setButton={(button) => {
                    setState({
