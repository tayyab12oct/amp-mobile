...
export default function StreamingServiceDetails({ props, location }) {
  const classes = useStyles();
  const { data, isSelected, onSelect, forPage } = location.state;
  const { width } = React.useContext(ViewportContext);
  console.log('location.state.data.name: ', location.state.data.name);
  const {
    webfox,
    webstore,
    actions,
  ...
  React.useEffect(() => {
    ...
    firebaseAnalytics.logEvent('StreamingServiceDetails', {
      screen_view:
    ...
    windowAny.Moengage && windowAny.Moengage.track_event('StreamingServiceDetails', {
      screen_view: '/streamingServicedetails',
  ...
  return (
    <div className={classes.root}>
      <Grid xs={12} container>
        ...
        <Grid xs={12} sm={10} lg={8} className={classes.containerBox} item>
          ...
          <DetailsCard
            ...
            logo={location.state.data.icon_url}
      ...
      <Grid xs={12} container className={classes.container2}>
        <Grid sm={1} lg={2} item></Grid>
        <Grid xs={12} sm={10} lg={8} className={classes.containerBox} item>
          <CommonDetails headerText={'Contents'} />
        </Grid>
        <Grid sm={1} lg={2} item></Grid>
      </Grid>
    </div>
