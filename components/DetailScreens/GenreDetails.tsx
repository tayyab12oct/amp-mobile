...
export default function GenreDetails({ props, location }) {
  const {
    webfox,
    webstore,
    actions,
  ...
  React.useEffect(() => {
    ...
    firebaseAnalytics.logEvent('GenreDetails', {
      screen_view:
        '/genreDetails' +
    ...
    windowAny.Moengage && windowAny.Moengage.track_event('GenreDetails', {
      screen_view: '/genreDetails',
  ...
  return (
    <div className={classes.root}>
      ...
      <Grid xs={12} container className={classes.container2}>
        <Grid sm={1} lg={2} item></Grid>
        <Grid xs={12} sm={10} lg={8} className={classes.containerBox} item>
          <CommonDetails headerText={location.state.data.name + ' Contents'} />
        </Grid>
        <Grid sm={1} lg={2} item></Grid>
      </Grid>
    </div>
